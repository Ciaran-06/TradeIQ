...
namespace MathUtils {
    ...
    double variance(const std::vector<double>& data, bool sample) {
        if (data.size() < (sample ? 2 : 1)) throw std::invalid_argument("Not enough data.");
        double m = mean(data);
        double sum = 0.0;
        for (double d : data) {
            double diff = d - m;
    ...
    double skewness(const std::vector<double>& data) {
        if (data.size() < 3) throw std::invalid_argument("Need at least 3 data points.");
        double m = mean(data);
    ...
    double kurtosis(const std::vector<double>& data) {
        if (data.size() < 4) throw std::invalid_argument("Need at least 4 data points.");
        double m = mean(data);
        double sd = standardDeviation(data, true);
        double k = 0.0;
        for (double d : data) {
    ...
    double percentile(std::vector<double> data, double p) {
        if (data.empty()) throw std::invalid_argument("Data is empty.");
        if (p < 0.0 || p > 100.0) throw std::invalid_argument("Percentile must be between 0 and 100.");
        std::sort(data.begin(), data.end());
    ...
    std::vector<double> zScoreNormalize(const std::vector<double>& data) {
        ...
        if (sd == 0.0) throw std::domain_error("Standard deviation is zero.");
